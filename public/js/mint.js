var mint_count = 1;
var chainId = 56; // Change it to 97 for test
var t_address = '0xD9F0A1720f3eEe530Ca4Dac532044450109c43e6'; // address
    ...
    web3.eth.getAccounts().then(async (tx) => {
        if (tx[0] !== undefined) {
            ...
            if (window.ethereum.networkVersion == chainId) {
                ...
                $('#mint_btn').html("Connect to BSC")
            }
        } else {
            $('#mint_link').html('MINT');
            $('#mint_btn').html('Connect Wallet');
        }
    });
}
...
async function trigger_minting(amt) {
    ...
    await contract.methods.mint(String(amt)).send({ from: selectedAccount, value: amt, gasLimit: "28500" }).then(function (res, err) {
        if (res) {
