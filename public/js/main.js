...
function startup(){
    ...
    if (window.ethereum) {
        ...
        async function presaleMint(amt, amt_pay){
            ...
            await contract.methods.presaleMint(String(amt)).send({ from: user_address, value: amt_pay, gasLimit:"28500", }).then(function (res, err) {
                if(res){
    ...
    web3.eth.getAccounts().then(async (tx) => {
        if(tx[0] !== undefined){
            ...
            if(window.ethereum.networkVersion == 1){
                ...
                $('#mint_btn').html("Connect to ETH Mainnet")
            }
